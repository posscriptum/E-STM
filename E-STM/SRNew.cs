...
namespace E_STM
{
    class SRNew: ElementNew
    {
        private ConnectorNew InternalConnector;
        private ConnectorNew InverseInternalConnector;
        public SRNew(string Name, Point Location, Panel PlaceForElement):base(Name, Location, PlaceForElement)
        {
        ...
        public override void implementation()
        {
            ...
            if (ElementConectors[1].Value > 0 && ElementConectors[0].Value <= 0)
            {
                if (InternalConnector != null)
                {
